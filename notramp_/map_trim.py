...
def run_map_trim(**kw):
    ...
    primers = fun.create_primer_objs(kw["primers"], kw["name_scheme"])
    amps, av_amp_len = fun.generate_amps(primers)
    out_paf = fun.name_out_paf(kw["reads"], kw["reference"], "trim")
    mm2_paf = fun.map_reads(kw["reads"], kw["reference"], out_paf, kw["seq_tec"])
    mappings = fun.create_filt_mappings(mm2_paf, av_amp_len, kw["set_min_len"], kw["set_max_len"])
    amps_bin_maps = bin_mappings(amps, mappings, kw["margins"])
    loaded_reads = fun.load_reads(kw["reads"])
    amps_bin_reads = load_amps_with_reads(amps_bin_maps, loaded_reads)
    clipped_out = fun.name_out_reads(kw["reads"], "clip", kw["out_dir"])
